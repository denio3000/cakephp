 ...
 */
	public function testLoginRedirect() {
		...
		array_push($this->Controller->methods, 'view.ctp', 'edit', 'index');
		...
		$this->Auth->loginAction = array('controller' => 'AuthTest', 'action' => 'login');
		$this->Auth->startup($this->Controller);
		$expected = Router::normalize('Posts/view.ctp/1');
		$this->assertEquals($expected, $this->Auth->Session->read('Auth.redirect'));
		...
		$_SERVER['HTTP_REFERER'] = 'http://webmail.example.com/view.ctp/message';
		$this->Auth->Session->delete('Auth');
		$url = '/Posts/edit/1';
